---
title: "Wrangle CANSIM Table 17-10-0005-01"
author: "Joel Barnes"
...
d <- get_cansim("17-10-0005-01")
```

```{r echo = FALSE}
d <- readRDS("c:/users/joelb/onedrive/github/cansim/CANSIM Table 17-10-0005-01.rds")
```
...
```{r}
# Retain select groups
d <- d %>% filter(
  `Age group` %in% c(paste0(seq(from = 20, to = 95, by = 5), " to ", seq(from = 24, to = 99, by = 5), " years"), "All ages", "100 years and over")
)
```
...
```{r}
# Retain years 2001 to 2021 
d <- d %>% filter(REF_DATE >= 2001, REF_DATE <= 2021)
```
...
```{r}
write.csv(d, "c:/users/joelb/onedrive/github/cansim/CANSIM Table 17-10-0005-01 - wrangled.csv", na = "", row.names = FALSE)
```